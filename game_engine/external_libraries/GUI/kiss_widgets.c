	...
	int x, int y, int w, int h)
{
	...
	if (!button->font.font) button->font = kiss_buttonfont;
	...
	kiss_makerect(&button->rect, x, y, w, h);
	button->textcolor = kiss_white;
	kiss_string_copy(button->text, KISS_MAX_LENGTH, text, NULL);
	button->textx = x + button->rect.w / 2 - kiss_textwidth(button->font, text, NULL) / 2;
	button->texty = y + button->rect.h / 2 - button->font.fontheight / 2;
	button->active = 0;
	button->prelight = 0;
	button->visible = 0;
	button->focus = 0;
...
int kiss_button_draw(kiss_button *button, SDL_Renderer *renderer)
{
	...
	if (button->active){
		kiss_renderimage_precise(renderer, button->activeimg, button->rect.x,
